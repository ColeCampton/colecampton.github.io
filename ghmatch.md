...
$$
\begin{align*}
    \argmin_{\Delta} \quad & \vecM(\Delta)^T P \vecM(\Delta) - \vecM(W)^T\vecM(\Delta)\\
    \text{subject to }& C\vecM(\Delta) = \mathbbm{1}\\
    & \Pi^T\Pi \vecM(\Delta)  = \vecM(\Delta)\\
    &  0\le \vecM(\Delta)\le 1
\end{align*}
$$

Where $$C \triangleq \begin{bmatrix}I_{n}\otimes \mathbbm{1}_{n}^T \\ \mathbbm{1}_{n}^T\otimes I_{n}\end{bmatrix}$$ is the $$2n\times n^2$$ unsigned incidence matrix which enforces the matrix relaxation of the permutation to be doubly stochastic for right-hand-side $\mathbbm{1}$, the column vector of all ones. 

The matrix $$P$$ is derived from a set of identities for the Frobenius norm $$\|\cdot\|_F$$, Kronecker product $$\otimes$$ and vectorization operator $$\vecM$$, which stretches a $$n\times m$$ matrix column-wise into a vector.
$$
\begin{align*}
\|\cdot\|_F^2 &= \|vecM(\cdot)\|_2^2 = \vecM(\cdot)^T\vecM(\cdot)\\
\vecM(ABC) &= C^T\otimes A \vecM(B)
\end{align*}
$$
...
$$N \triangleq \begin{bmatrix}0 \\ - I\end{bmatrix}, \quad M \triangleq \begin{bmatrix}C \\ I \end{bmatrix},\quad b\triangleq \begin{bmatrix} \mathbbm{1}\\0\end{bmatrix}$$
